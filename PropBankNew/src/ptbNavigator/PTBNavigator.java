...
/* CONSTRUCTORS */
	/** Default constructor. */
	public PTBNavigator() throws SAXException
	{
		
	}
	
	/** This Constructor takes in a single string 'root' 
	 * for the path to the desired file
	 * 
	 * It will throw a SAXException if it cannot successfully construct
	 * an internal GraphParser, and an IOException if it cannot find a 
	 * file at the given path
	 * 
	 * Note: do not include the extension. e.g. if the file were
	 * "117CWL008.mrg" give the string "path/117CWL008", not 
	 * "path/117CWL008.mrg"
	 * 
	 * @param root String the path to the desired file w/o the extension
	 * @throws SAXException, IOException
	 */
	public PTBNavigator(String rt) throws SAXException, IOException
	{
...
/* PRIVATE MUTATOR/INITIALIZER FUNCTIONS */
	/**  initializes the graph representation for the file given as 
	 * input to {@link #PTBNavigator(String)}
	 * throws SAXException, IOException
	 */
	private void setGraph() throws SAXException, IOException
	{
		/* initialize the graphParser 
		 * NOTE: Throws SAXException
		 */
		graphParse = new GraphParser();
		
		/* use the graph parser to create
		 * the PTBGraph
		 */
		ptbGraph = graphParse.parse(root+PTB_EXTENSION);
		
		/* now that we have a graph, call the setSentences method
		 * to initialize our sentences
		 */
		setSentences();
		
		/* now that we have a graph, call the setAugmentedSentences
		 * method to initialize our sentences
		 * TODO: replace/merge this with setSentences()
		 */
		setAugmentedSentences();
	}
	
	/** initializes a reader for the source text of the file 
	 * given as input to {@link #PTBNavigator(String)}
	 * @throws IOException
	 */
	private void setSource() throws IOException
	{
		/* try to initialize a UTF8 reader using the given file extension
		 * if this doesn't work, print out the stack trace for now
		 * TODO: try-catch block with more helpful error message?
		 */
		source = new UTF8Reader(root+SOURCE_EXTENSION);
		
		/* grab the source file as a single string
		 * TODO: it may be more efficient to do this only
		 * when testing, since the string will be quite 
		 * large
		 */
	...
	/** initializes the sentences list by 
	 * retrieving all the sentence nodes from the PTB 
	 * representation of the file given as input to 
	 * {@link #PTBNavigator(String)}
	 */
	private void setSentences()
	{
		//get the root of the ptbGraph
		INode rootNode = ptbGraph.getRoot();
	...
	/** initializes the augmented sentences list
	 * by retrieving all the sentence nodes from 
	 * the PTB representation of the file given 
	 * as input to {@link #PTBNavigator(String)}
	 * transforming them into AugmentedSentenceNodes,
	 * and sorting them by their start fields
	 */
	private void setAugmentedSentences()
	{
		//get the root of the PTB Graph
		INode rootNode = ptbGraph.getRoot();
		
		/* add all of the child nodes, which at 
		 * this level should be the sum total of
		 * independent entities (sentences) from 
		 * the source
		 */
		for( IEdge e : rootNode.getOutEdges())
		{
			/** Each sentence node is converted into an AugmentedSentenceNode
			 * which, upon initialization, sets its start and end anchors
			 * which will be used to sort the sentences
			 */
			AugmentedSentenceNode sentence = new AugmentedSentenceNode(e.getTo(), sourceText);
	...
	/** Sorts the sentences retrieved from the PTB representation
	 * by their start anchors (which were included when they were
	 * converted into AugmentedSentenceNodes)
	 */
...
/* MAIN UTILITY FUNCTIONS */
	...
	/** This procedure looks in the directory for the 
	 * address indicated by the input, root, and 
	 * initializes the source text and PTBGraph for 
	 * the PTBNavigator. When processing files from 
	 * multiple different texts, use this function to 
	 * switch from one text to the next.
	 */
 	public void setRoot(String root) throws IOException, SAXException
	{
		//set the root directory to the given directory
		this.root = root;
	...
	/**This signals the PTBNavigator to operate
	 * at the sentence whose number is given as 
	 * input
	 *@param index the "index"th sentence in the PTB
	...
	/**This signals the PTBNavigator to travel
	 * n number of tokens in its given sentence
	 */
	...
	/** This method presumes that the sentence has already
	 * been specified and navigates within the sentence
	 * using the first input, tokenAcross, to move that 
	 * number of leaves along the sentence's tree, and then
	 * it moves "up" the tree using the second input, parentUP.
	 * 
	...
	/** This is the main method to navigate within a PTB text.
	 * The order of parameters is sentence, indicating the 
	 * index of the sentence within the text; tokenAcross, 
	 * indicating the number of leaves to travel along a tree 
	 * representation of the sentence; and parentUp, the number
	 * of nodes "up" from the leaf indicated by tokenAcross.
	 * 
	 * @param sentence - index of the sentence within the PTB representation
	 * @param tokenAcross - the horizontal position of the desired node
	 * @param parentUp - the vertical position of the desired node
	 * @return PTBNodeResult
	 */
...
/* DISPLAY FUNCTIONS */
	...
	/** Print out the sentences that the PTBNavigator has discovered
	...
	public void printSentences()
	{
		...
		for(INode n : sentences)
		{
			...
			/* ---- DISPLAY A LIST OF OUT EDGES ----
				System.out.print("\t"); //indentation sets off list of out edges
	...
	public void printCurrentSentence()
	{
		...
		System.out.println(
							"===================CURRENT SENTENCE=========\n"+
							"NUMBER: "+sentIndex+
							"  "+"TYPE: "+sentence.getAnnotation().getLabel() +
							"\nTEXT: "+ sourceText.substring(   //extract the target text from the source file
	...
	/** Print out the original sentences for comparison 
	 *  TODO: this should not be necessary after the sort function is working
	 */
	...
	/** Print out the augmented sentences 
	 * which provides detailed information about
	 * their anchors in the source text and 
	 * displays the actual text they annotate
	 */
	...
	public void printCurrentToken()
	{
		...
		System.out.println("Trace: "); printTrace(token);
		System.out.println("============================================");
	}
	
	/** returns the string corresponding to the text that anchors the token
	 * that the PTBNavigator is set at
	 * @return currTokText
	 */
	protected String currentTokenText()
	{
		//get the token's anchors
		CharacterAnchor tokS = (CharacterAnchor) token.getLinks().get(0).getRegions().get(0).getAnchor(0);
		CharacterAnchor tokE = (CharacterAnchor) token.getLinks().get(0).getRegions().get(0).getAnchor(1);
		
		return sourceText.substring(tokS.getOffset().intValue(), tokE.getOffset().intValue());
	}
	

	/* DIAGNOSIS FUNCTIONS */
	
	/** Prints out information about the sentence at the given index
	 * including:
	 *    1. the order in which the token occurs in the sentence(the "NUM" column)
	 *    2. whether the sentence was marked as the start or end of the sentence
	 *      (the "S/E" column)
	 *    3. the text string corresponding to the token (the "TOKEN-VALUE" column)
	 *    4. the anchor values for the token within the text (the "ANCHORS" column)
	 * 
	 * Following this table, the procedure looks at the start and end tokens and 
	 * traces their parentage
	 * 
	 * Typically, if there are issues, there will either be multiple tokens labeled
	 * "#S#" or "#E#" and their anchors will be drastically different from other 
	 * tokens in the sentence. These are usually caused by "traces", in which a 
	 * word refers to another word earlier in the sentence (e.g. the demonstrative
	 * "these" in the previous sentence, referring to the issues in the sentence 
	 * preceding it) 
	 */
	public void diagnose(int sentenceIndex)
	{
		...
		/* print out information about the sentence in question */
			...
			{
				//get the token's anchors
				CharacterAnchor tokS = (CharacterAnchor) tok.getLinks().get(0).getRegions().get(0).getAnchor(0);
				CharacterAnchor tokE = (CharacterAnchor) tok.getLinks().get(0).getRegions().get(0).getAnchor(1);
				
				//print out the token's index
				System.out.printf("| %-3d ", tokIndex++ );
				
				/* If the token marks a start or end index,
				 * i.e. is at the beginning or end of the sentence,
				 * record that, otherwise, print the equivalent
				 * number of spaces
				 */
				...
				System.out.printf("|   %-15s",
						sourceText.substring(tokS.getOffset().intValue(), (Integer)tokE.getOffset().intValue()));
		...
		//---START TOKEN DIAGNOSIS---
			...
			INode startToken = patient.getStartNode(); 
				...
				CharacterAnchor stEndAnchor = (CharacterAnchor) startToken.getLinks().get(0).getRegions().get(0).getAnchor(1);
			...
			System.out.println("TOKEN: "+sourceText.substring((int) patient.getStart(), stEnd));
		...
		//---END TOKEN DIAGNOSIS---
			...
			// get the startToken's end anchor
			CharacterAnchor etStartAnchor = (CharacterAnchor) endToken.getLinks().get(0).getRegions().get(0).getAnchor(0);
			// use the end anchor to get the integer offset
			...
			System.out.println("TOKEN: "+sourceText.substring(etStart, (int) patient.getEnd()));
			System.out.println("TRACE: ");
			printTrace(endToken);
			System.out.println("-----------------");
		//---------------------------
	...
	/** Prints out a trace of the navigation indicated by 
	 * the given indices, assumes that the sentence 
	 * has already been set
	 * @param tokenAcross - # of leaves to move horizontally
	 * @param parentUp - # of nodes to move up vertically
	 */
	public void navigateTrace(int tokenAcross, int parentUp)
	{
		...
		System.out.println("Move ["+sentIndex+"]("+tokenAcross+","+parentUp+")");
	 ...
	 * the given indices, assumes that the sentence 
	 * has already been set
	 * @param sentence - # sentence in the text
	 * @param tokenAcross - # of leaves to move horizontally
	 * @param parentUp - # of nodes to move up vertically
	 */
	public void navigateTrace(int sentence, int tokenAcross, int parentUp)
	{
		...
		System.out.println("=======NAVIGATE TRACE=======");
		System.out.println("Move ["+sentence+"]("+tokenAcross+","+parentUp+")");
		printTraceIndexed(this.token, parentUp);
	 ...
	 * 
	 * @param target INode the base token to trace upward on a graph
	 */
	public void printTrace(INode target)
	{
		...
		{
			System.out.println("\t"+index++ + ": " + temp.getAnnotation().getLabel());
			temp = temp.getParent();
		}
	}
	...
	/** this method takes in an index and travels that many levels up from the start token	 * 
	 * @param index int how far up to trace from the token given
	 */
...
/* LOCAL VARIABLES */
	...
	/* Variables involved in constructing the source files */
		...
		/** a graph parser to handle the PTB representation */
			protected GraphParser graphParse = new GraphParser();
		
		/** the graph that will serve as the main construct over which we are navigating */
			protected IGraph ptbGraph;
	...
	/* Variables involved in sentence organization */
		...
		/** empty collection for the original order of the sentences 
		 * to be preserved after the sentences are sorted by the 
		 * AnchorComparator.
		 * TODO: hopefully this won't be necessary in the future, but
		 * for now, the sentences from the PTB representation need to 
		 * be organized locally
		 */
			protected ArrayList<INode> unOrganizedSentences = new ArrayList<INode>();
			
		/** empty collection for all the sentences in the PTB representation 
		 * after being converted to AugmentedSentenceNodes
		 */
